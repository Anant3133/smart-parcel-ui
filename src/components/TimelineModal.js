...
export default function TimelineModal({ timeline, onClose, handlersMap = {} }) {
  ...
  const logs = [
    ...
    ...(timeline.handovers?.map(log => ({
      action: log.status,
      location: handlersMap[log.location] || log.location,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="text-black bg-white p-6 rounded shadow-lg max-w-2xl w-full">
        ...
        {logs.length === 0 ? (
          ...
          <table className="w-full text-left border">
            ...
            <tbody>
              {logs.map((log, index) => (
                <tr key={index}>
                  <td className="text-black px-2 py-1 border">{log.action}</td>
                  <td className="text-black px-2 py-1 border">{log.location}</td>
                  <td className="text-black px-2 py-1 border">{new Date(log.handoverTime).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
